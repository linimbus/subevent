...
int main() {
  ...
  bus.Subscribe<MsgEvent>([](const MsgEvent &e) {
    std::cout << "[Single] Got: " << e.data << "\n";
  });

  bus.Subscribe<MsgEvent2>([](const MsgEvent2 &e) {
    std::cout << "[Single] Got value: " << e.value << "\n";
  });
  ...
  bus.Subscribe<MsgEvent2, MsgEvent3>(
      MsgEvent2{3.14}, MsgEvent3{true},
      [](const MsgEvent2 &e1, const MsgEvent3 &e2) {
        std::cout << "[Multi] Combined -> val: " << e1.value
                  << ", flag: " << e2.flag << "\n";
      });
